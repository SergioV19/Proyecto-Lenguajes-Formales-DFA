...
public class MyFrame extends JFrame {
	...
	private PanelGrafo grafo;
	private JPanel derechaJPanel;

	private int x;
	private int y;
	
	public MyFrame(Controller control, ArrayList<Estado> arrayList) {
		super("Automata: ");
		x = 75;
		y = 25;
		...
		grafo = new PanelGrafo(arrayList);
	...
	public void fillEstadosInicialesCheck(ArrayList<Estado> arrayList) {
		comboestadoInicial.removeAllItems();
		comboestadoFinal.removeAllItems();

		for (Estado estadiIni : arrayList) {
	...
	public Estado getEstado() {
		...
		if (cbg.getSelectedCheckbox().equals(inicial)) {
			x = x+100;
			return new Estado(textEstado.getText(), true, false, x, y);
		} else if (cbg.getSelectedCheckbox().equals(estadofinal)) {
			x = x+100;
			return new Estado(textEstado.getText(), false, true, x, y);
		} else {
			x = x+100;
			return new Estado(textEstado.getText(), false, false, x, y);
		}
	...
	public void updateGrafo(ArrayList<Estado> estados) {
